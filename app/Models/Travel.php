...
class Travel extends AbsData
{
    ...
    public function get_total_customer_offer($id_customer,$id_offer){
        $conn=new Mysql();
        $result=$conn->fetchData("SELECT SUM(km) AS som FROM `{$this->table}` WHERE id_customer=? AND id_offer=? AND id_user=?",[$id_customer,$id_offer,$_SESSION['user']->id]);
        return $result[0]['som'];
    }
}